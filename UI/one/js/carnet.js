...
function makepdf(imgData, textdata, coverdata)
{
  var doc = new jsPDF();
  doc.addFont('Courier');
  doc.setFont('Courier');
  ...
  function addCoverPage()
  {
      ...
      try
      {
        ...
        doc.setFontSize(8);
        doc.text("ACCORDING TO AUTHORITY / GEMÄSS VOLLMACHT", 28, 80);
        doc.text("PROFESSIONAL EQUIPMENT / BERUFSAUSRÜSTUNG", 28, 98);
      }
      catch(e)
      {
...
//    - if empty line with only description, make new line and dont try to calculate stuff ( gotta be done in the other script)
    ...
    //doc.save('a4.pdf'); // this saves the pdf and downloads it
    ...
    if (coverdata != undefined)
    {
      ...
      {
          doc.save('carnetFORM.pdf'); // this saves the pdf and downloads it
      }
    }
...
console.log(carnetArray); // includes the data for the pdf
  ...
  carnetArray = numberItems(carnetArray);
  console.log(carnetArray);
  makepdf(undefined, carnetArray, {companyname:"",street1:"",street2:"",zipcodecity:""});
